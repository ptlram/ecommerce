                    ...
                    <div class="col-md-12">
                        ...
                        <div class="portlet box green">
                            <div class="portlet-title">
                                ...
                                <a class="dt-button buttons-print btn default" style="margin-left: 2%; margin-top: 0.4%;" href="./productadd.php">ADD PRODUCT</a>
                            </div>
                                ...
                                <table
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($brand as $std) {
                                            echo '<tr>';
                                            echo '<td>' . $std['id'] . '</td>';
                                            echo '<td>' . $std['product_name'] . '</td>';
                                            echo '<td>' . $std['variant_name'] . '</td>';
                                            echo '<td>' . $std['brand'] . '</td>';
                                            $cid = $std['category'];
                                            ...
                                            $subcid = $std['subcategory'];
                                            $query = $pdo->prepare("SELECT * from subcategory where id=$subcid");
                                            $query->execute();
                                            $subcategory = $query->fetchAll();

                                            echo '<td>' . $subcategory[0]['name'] . '</td>';
                                            echo '<td>' . $std['base_price'] . '</td>';
                                            echo '<td>' . $std['mrp'] . '</td>';
                                            echo '<td>' . $std['retailer_price'] . '</td>';
                                            echo '<td>' . $std['wholesaler_price'] . '</td>';
                                            // echo '<td>' . $std['short_description'] . '</td>';
                                            // echo '<td>' . $std['long_description'] . '</td>';
                                            // echo '<td>' . $std['product_image'] . '</td>';
                                            // echo '<td>' . $std['product_image'] . '</td>';

                                            echo '<td style="display: flex; justify-content: center; align-items: center; gap: 15px;">
                                            <!-- Update Button -->
                                            <a style="color:black; padding: 10px;" href="./productupdate.php?id=' . $std["id"] . '">
                                                <span style="font-size: 1.5em;" class="glyphicon glyphicon-pencil"></span> 
                                            </a>
                                    
                                            <!-- Delete Button -->
                                            <a style="padding: 10px;" href="./productdelete.php?id=' . $std["id"] . '" 
